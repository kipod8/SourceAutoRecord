...
static void recordEntityOrgs() {
	for (int i = 0; i < Offsets::NUM_ENT_ENTRIES; ++i) {
		...
		playerTrace->EmitLog(Utils::ssprintf("abs org:  (%.6f,%.6f,%.6f)", org.x, org.y, org.z).c_str());
		org = ent->collision().GetCollisionOrigin();
		playerTrace->EmitLog(Utils::ssprintf("coll org: (%.6f,%.6f,%.6f)", org.x, org.y, org.z).c_str());
...
static void recordIvpEntityOrgs() {
	for (int i = 0; i < Offsets::NUM_ENT_ENTRIES; ++i) {
		...
		playerTrace->EnterLogScope(Utils::ssprintf("entity %d", i).c_str());
		playerTrace->EmitLog(classname);
		playerTrace->EmitLog(Utils::ssprintf("speed: (%.6f,%.6f,%.6f)", speed.x, speed.y, speed.z).c_str());
		playerTrace->EmitLog(Utils::ssprintf("pos:   (%.6f,%.6f,%.6f)", pos.x, pos.y, pos.z).c_str());
		playerTrace->ExitLogScope();
	}
}
...
#undef GET
#define EMIT_VEC(vec, name) playerTrace->EmitLog(Utils::ssprintf(name ": (%.6f,%.6f,%.6f)", vec.x, vec.y, vec.z).c_str())
#define EMIT_BOOL(val, name) playerTrace->EmitLog(Utils::ssprintf(name ": %s", (val) ? "true" : "false").c_str())
...
DECL_DETOUR_T(void, PostThinkVPhys) {
	...
	playerTrace->EmitLog(Utils::ssprintf("PRE  m_vNewVPhysicsVelocity=(%.6f,%.6f,%.6f)", vel->x, vel->y, vel->z).c_str());
	playerTrace->EmitLog(Utils::ssprintf("PRE  m_touchedPhysObject=%s", SE(thisptr)->fieldOff<bool>("m_flNextDecalTime", -4) ? "true" : "false").c_str());

	g_PostThinkVPhys_Hook.Disable();
	PostThinkVPhys(thisptr);
	g_PostThinkVPhys_Hook.Enable();

	playerTrace->EmitLog(Utils::ssprintf("POST m_vNewVPhysicsVelocity=(%.6f,%.6f,%.6f)", vel->x, vel->y, vel->z).c_str());
	playerTrace->EmitLog(Utils::ssprintf("POST m_touchedPhysObject=%s", SE(thisptr)->fieldOff<bool>("m_flNextDecalTime", -4) ? "true" : "false").c_str());
...
void RngManip::EnterProcessMovement(void *gamemovement, CMoveData *move) {
	...
	playerTrace->EmitLog(Utils::ssprintf("PRE  m_outWishVel=(%.6f,%.6f,%.6f)", v.x, v.y, v.z).c_str());
}
void RngManip::ExitProcessMovement(CMoveData *move) {
	auto v = move->m_outWishVel;
	playerTrace->EmitLog(Utils::ssprintf("POST m_outWishVel=(%.6f,%.6f,%.6f)", v.x, v.y, v.z).c_str());
	playerTrace->ExitLogScope();
}
...
DECL_DETOUR_T(void, AirAccelerate, Vector *wishdir, float wishspeed, float accel) {
	...
	playerTrace->EmitLog(Utils::ssprintf("wishdir=(%.6f,%.6f,%.6f)", wishdir->x, wishdir->y, wishdir->z).c_str());
	playerTrace->EmitLog(Utils::ssprintf("wishspeed=%.6f", wishspeed).c_str());
	playerTrace->EmitLog(Utils::ssprintf("accel=%.6f", accel).c_str());
